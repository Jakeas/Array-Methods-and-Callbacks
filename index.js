...
const filter2014 = fifaData.filter((final) =>{
...
const finalData = []
    data.forEach(function(finals){
        if(finals.Stage === "Final"){
            finalData.push(finals)
        } 
    }); 
    return finalData
}
// console.log("T2:", getFinals(fifaData))
...
function getYears(arr, getFinals) {
    const years = getFinals(arr).map(function(item){  
           return item.Year
    });
    return years
...
function getWinners(arr, getFinals) {
    const winners = getFinals(arr).map(function(item){
...
function getAverageGoals(getFinalsCB) {
    let average = getFinalsCB.reduce((counter, item) => counter + (item["Home Team Goals"] + item["Away Team Goals"] ), 0) / getFinalsCB.length;
